...
# 1. 标准半监督
echo "Step 1/5: Running Standard..." | tee -a "$RESULT_FILE"
python main.py --dataset "$DATASET" --mode standard --epochs "$EPOCHS" >> "$RESULT_FILE" 2>&1

# 2. 不平衡-无权
echo "Step 2/5: Running Imbalanced..." | tee -a "$RESULT_FILE"
python main.py --dataset "$DATASET" --mode imbalanced --epochs "$EPOCHS" >> "$RESULT_FILE" 2>&1

# 3. 不平衡-加权
echo "Step 3/5: Running Weighted..." | tee -a "$RESULT_FILE"
python main.py --dataset "$DATASET" --mode weighted --epochs "$EPOCHS" >> "$RESULT_FILE" 2>&1

# 4. 运行原始 IceBerg (无增强)
echo "Step 4/5: Running IceBerg (Original)..." | tee -a "$RESULT_FILE"
python main.py --dataset "$DATASET" --mode iceberg_original --epochs "$EPOCHS" --mu "$MU" --lam "$LAM" >> "$RESULT_FILE" 2>&1

# 5. 运行 IceBerg+ 增强版 (Ours)
echo "Step 5/5: Running IceBerg+ (Ours)..." | tee -a "$RESULT_FILE"
